...
public class MeetinginfoDaoImpl implements MeetinginfoDao {
	...
	public int addMeetinginfo(MeetinginfoBean meetinginfoBean) throws Exception {
		int rows = 0;
		connection = dbUtil.getConnection();
		String sql = "insert into meetinginfo(meetingname,numofattendents,startdate,enddate,capacity,accountname,description,selectEmployees) values (?,?,?,?,?,?,?,?)";
		preparedStatement = connection.prepareStatement(sql);
		...
		preparedStatement.setString(8, meetinginfoBean.getSelectEmployees());
	...
	public int fetchMeetinginfoRows(MeetinginfoBean meetinginfoBean)
		...
		connection = dbUtil.getConnection();
		String sql = "select count(*) from meetinginfo where meetingname like ? and capacity like ? and accountname like ? and adddate like ? and startdate like ? and enddate like ?";
		preparedStatement = connection.prepareStatement(sql);
